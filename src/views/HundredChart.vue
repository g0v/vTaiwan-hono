...
const rows = computed(() => Array.from({ length: 10 }, (_, row) => Array.from({ length: 10 }, (_, col) => row * 10 + col + 1)))
</script>

<template>
  <div class="hundred-root">
    <main class="hundred-main container">
      <h1>百數表 · Hydration 示範</h1>
      <p class="lead">
        此頁在 Worker 端 SSR 後，由瀏覽器載入 client bundle 進行
        <strong>hydration</strong>。下方使用 <code>v-model</code>、雙層 <code>v-for</code>、以及 <code>:style</code> 動態著色，可在無重新整理的情況下互動。
      </p>

      <section class="hundred-controls" aria-label="倍數著色">
        <label class="control-label" for="factor-input"> 倍數（輸入正整數，例如 3，將 3 的倍數格以淺紫底色標示） </label>
        <input id="factor-input" v-model="factorRaw" type="number" min="1" step="1" inputmode="numeric" class="factor-input" placeholder="例如 3" />
      </section>
