...
const filteredTopicsBigImg = computed(() => filteredTopics.value.filter(topic => topic.status === '即將開始' || !topic.status))

const filteredTopicsSmallImg = computed(() => filteredTopics.value.filter(topic => topic.status !== '即將開始'))
...
<template>
  <section class="vt-hero-bg mt-[-86px] text-white sm:mt-[-90px]">
    ...
    <div class="mx-auto px-2">
      <div class="mx-auto max-w-7xl">
        ...
        <div v-else-if="recentTopics.length > 0" class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3">
          <div v-for="topic in recentTopics" :key="topic.id" class="vt-topic-card min-h-[260px] max-w-sm cursor-pointer p-6" @click="goToTopic(topic)">
            ...
            <div class="mb-5 flex items-start gap-4 pr-10">
              ...
              <div class="min-w-0">
                <h3 class="mb-2 line-clamp-2 text-[19px] leading-tight font-bold tracking-[-0.01em] text-vt-gray-800">
                  {{ topic.title }}
                </h3>
            ...
            <p v-if="topic.slogan" class="mb-4 line-clamp-2 text-sm leading-[1.55] text-vt-fg-2">
              {{ topic.slogan }}
            </p>
    ...
    <div class="mx-auto px-2">
      <div class="mx-auto flex max-w-7xl flex-col items-start gap-4 lg:flex-row lg:items-center">
        <div class="max-w-md flex-1">
          <div class="relative">
            <input
              ...
              class="w-full min-w-64 rounded-lg border border-gray-300 bg-white px-4 py-3 pr-12 pl-12 text-gray-900 focus:ring-2 focus:ring-jade-green focus:outline-hidden"
            />
            <IconWrapper name="search" :size="20" class="absolute top-1/2 left-4 -translate-y-1/2 transform text-gray-400" />
            <button v-if="searchQuery" class="absolute top-1/2 right-4 -translate-y-1/2 transform text-gray-400 transition-colors hover:text-gray-600" @click="clearSearch">
        ...
        <div class="relative mx-auto mt-4 w-64 lg:mt-0">
          ...
          <span class="pointer-events-none absolute top-1/2 right-3 -translate-y-1/2 transform">
      ...
      <div class="mx-auto max-w-7xl">
        ...
        <div v-else-if="filteredTopics.length > 0">
          <div v-if="filteredTopicsBigImg.length > 0" class="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div v-for="topic in filteredTopicsBigImg" :key="topic.id" class="vt-topic-card min-h-[320px] w-full p-6">
              ...
              <div class="mb-5 flex items-start gap-4 pr-10">
                ...
                <div class="min-w-0">
                  <h3 class="mb-2 line-clamp-2 text-2xl leading-tight font-bold tracking-[-0.01em] text-vt-gray-800">
              ...
              <div v-if="topic.cover" class="vt-topic-cover-slice relative mb-5 min-h-[112px]" :style="{ backgroundImage: `url(${topic.cover})` }" />

              <div class="mb-4 flex flex-1 flex-col">
                <p v-if="topic.slogan" class="mb-5 line-clamp-3 text-base leading-[1.65] text-vt-fg-2">
                  {{ topic.slogan }}
                </p>
          ...
          <div v-if="filteredTopicsSmallImg.length > 0" class="grid grid-cols-1 gap-0 md:grid-cols-2 md:gap-6 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
            <div v-for="topic in filteredTopicsSmallImg" :key="topic.id" class="vt-topic-card min-h-[190px] cursor-pointer p-5" @click="goToTopic(topic)">
              ...
              <div class="mb-4 flex items-start gap-3 pr-10">
                ...
                <div class="min-w-0">
                  <h3 class="mb-2 line-clamp-2 text-[17px] leading-tight font-bold tracking-[-0.01em] text-vt-gray-800">
                    {{ topic.title }}
                  </h3>
                  <div class="vt-topic-eyebrow flex-wrap">
                    <span class="vt-topic-status-dot" aria-hidden="true" />
        ...
        <div v-else class="py-12 text-center">
          ...
          <button v-if="searchQuery" class="vt-btn vt-btn-primary mt-4" @click="clearSearch">
