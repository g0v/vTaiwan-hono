    ...
    <div class="mx-auto max-w-4xl">
      ...
      <div class="mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4">
        <div class="flex items-center space-x-3">
          <img :src="'/CC_BY_SA.png'" alt="CC-BY-SA-4.0" class="h-8 w-auto" />
          <div class="text-sm text-blue-800">
            ...
            <p class="mt-1 text-xs">
              您可以自由分享、修改本內容，惟需標註原作者並以相同授權條款分享
            </p>
          </div>
        </div>
      </div>
...
function getDateTime(message: string): string {
  return message.split("\n")[0].replace(/\].+?$/, "]");
}

function dropSpeakerAndDateTime(message: string): string {
...
// 資料抓取在 onMounted（SSR 不執行，避免瀏覽器 API 問題）
onMounted(async () => {
  try {
    loading.value = true;
    error.value = "";
