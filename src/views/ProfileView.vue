...
const profileName = computed(
  () => props.userData?.name || props.user?.displayName || t("profile.notSet"),
);
...
async function saveProfile() {
  ...
  try {
    ...
    const { auth, database, databaseRef, databaseUpdate, updateProfile } =
    ...
    <section class="rounded-vt-lg border border-vt-border bg-vt-bg-1 p-6 shadow-vt-md sm:p-8">
      ...
      <div v-else-if="!editing" class="space-y-8">
        <div class="flex items-center gap-4">
          <img
            ...
            class="h-16 w-16 rounded-vt-full border border-vt-border object-cover"
          />
          <div
            v-else
            class="flex h-16 w-16 items-center justify-center rounded-vt-full bg-vt-bg-2 text-vt-fg-2"
            aria-hidden="true"
          >
            👤
          </div>
        ...
        <div class="flex flex-wrap gap-3">
          ...
          <button
            type="button"
            class="vt-btn rounded-vt-md border border-democratic-red text-democratic-red hover:bg-vt-red-tint"
            @click="emit('logout')"
          >
            {{ t("common.logout") }}
          </button>
      ...
      <div v-else class="space-y-6">
        ...
        <form class="space-y-5" @submit.prevent="saveProfile">
          <div>
            <label for="display-name" class="mb-2 block font-sans text-vt-sm font-medium">{{
              t("profile.name")
            }}</label>
            <input
              id="display-name"
          ...
          <div class="flex flex-wrap gap-3">
            <button
              type="submit"
              class="vt-btn vt-btn-primary disabled:cursor-not-allowed disabled:opacity-50"
            ...
            <button
              type="button"
              class="vt-btn rounded-vt-md border border-vt-border text-vt-fg-2 hover:bg-vt-bg-2"
              :disabled="updating"
              @click="cancelEdit"
            >
              {{ t("common.cancel") }}
