    ...
    <div class="container py-8">
      <div class="mb-8 flex flex-col items-center justify-between md:flex-row">
        ...
        <p class="text-sm text-gray-300">
          ...
          <a :href="`https://medium.com/@${MEDIUM_USERNAME}`" target="_blank" rel="noopener noreferrer" class="text-sm text-democratic-red hover:underline">Medium/@{{ MEDIUM_USERNAME }}</a>
        </p>
      </div>
      ...
      <div v-else-if="error" class="py-8 text-center">
        ...
        <button @click="loadArticles" class="rounded-md bg-democratic-red px-4 py-2 text-white transition hover:opacity-90">
          {{ t('medium.retry') }}
        </button>
      </div>
      ...
      <div v-else-if="articles.length > 0" class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        <article v-for="article in articles" :key="article.id" class="rounded-lg bg-white p-6 shadow-md transition-shadow hover:shadow-lg">
          ...
          <h2 class="mb-3 text-xl font-bold">
            <a :href="article.link" target="_blank" rel="noopener noreferrer" class="text-gray-900 transition-colors hover:text-democratic-red">
              {{ article.title }}
            </a>
          </h2>
          ...
          <div v-if="article.categories.length > 0" class="mb-4 flex flex-wrap gap-2">
            <span v-for="category in article.categories" :key="category" class="rounded-full bg-jade-green/10 px-2 py-1 text-sm text-jade-green">#{{ category }}</span>
          </div>

          <!-- 外部連結 -->
          <div class="mt-4">
            <a :href="article.link" target="_blank" rel="noopener noreferrer" class="text-sm font-medium text-democratic-red hover:underline">{{ t('medium.readMore') }} →</a>
          </div>
        </article>
      </div>
