<template>
  <div class="container mx-auto px-4 py-8">
    <div class="mx-auto max-w-6xl">
      ...
      <div v-if="props.user" class="mb-8 rounded-lg bg-white p-6 shadow-md">
        ...
        <div class="space-y-4">
          ...
          >
            {{
              uploading
                ? t("transcriptions.upload.uploading")
                : t("transcriptions.upload.uploadButton")
            }}
          </button>
        </div>
      </div>
      ...
      <div
        v-if="error"
        class="mb-6 rounded-sm border border-red-400 bg-red-100 px-4 py-3 text-red-700"
      >
        {{ error }}
      </div>
      ...
      <div v-if="!loading && transcriptions.length > 0" class="space-y-4">
        ...
        <div class="flex flex-col-reverse gap-4">
          ...
          >
            <!-- 樣稿標籤 -->
            <div v-if="item.meeting_id === '20250621'" class="absolute -right-2 -top-2 z-10">
              <div
                class="rotate-12 transform bg-yellow-400 px-3 py-1 text-xs font-bold text-black shadow-md"
              >
            ...
            <div class="flex items-start justify-between">
              <div class="flex-1">
                <h3 class="mb-2 text-lg font-semibold text-gray-900">
                  {{ t("transcriptions.list.meetingId") }}: {{ item.meeting_id }}
                </h3>
                <div class="mb-4 text-sm text-gray-600">
                  <img :src="'/CC0.png'" alt="CC0" class="h-8 w-auto" />
                  <!-- eslint-disable-next-line vue/no-v-html -->
                  <div
                    v-html="getRenderedOutlinePreview(item.outline)"
            ...
            <div class="mt-2 text-xs text-gray-500">
              {{ t("transcriptions.list.fileName") }}: transcript-{{
                formatMeetingId(item.meeting_id)
              }}.txt
            </div>
          </div>
        </div>
      ...
      <div class="max-h-[80vh] w-full max-w-4xl overflow-y-auto rounded-lg bg-white" @click.stop>
        ...
        <div class="max-h-[60vh] overflow-y-auto p-6">
          ...
          <textarea
            v-else
            v-model="myOutline"
            class="h-full max-h-[60vh] min-h-[200px] w-full"
          ></textarea>
        </div>

        <div
          class="flex flex-col items-center justify-between border-t border-gray-200 p-6 md:flex-row"
        >
          <button
            @click="copyOutline"
            class="flex items-center space-x-2 rounded-md bg-blue-600 px-4 py-2 text-white hover:bg-blue-700"
          >
          ...
          >
            ...
            <svg v-else class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                ...
                stroke-linejoin="round"
                stroke-width="2"
                d="M5 5v14h14V5H5zm4 0v4h6V5H9zm0 6v6h6v-6H9z"
              />
            </svg>
            <span>{{
              editing
                ? t("transcriptions.outline.saveAndEndEdit")
                : t("transcriptions.outline.edit")
            }}</span>
          </button>
          <button
            v-if="editing"
...
const isAdmin = computed(
  () => props.userData?.isAdmin === true || props.userData?.isSuperAdmin === true,
);
...
const filteredTranscriptions = computed(() =>
  transcriptions.value.filter(
    (item) => item.meeting_id.includes(search.value) || item.outline.includes(search.value),
  ),
);
  ...
  try {
    ...
    if (response.ok) {
      ...
      const idx = transcriptions.value.findIndex(
        (item) => item.meeting_id === currentOutlineMeetingId.value,
      );
