  ...
  <div class="pt-20">
    <div class="container mx-auto">
      ...
      <div v-else-if="newsletters.length" class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        <article v-for="newsletter in newsletters" :key="newsletter.id" class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xs transition hover:-translate-y-0.5 hover:shadow-lg">
          <RouterLink :to="{ name: 'newsletter-detail', params: { slug: newsletter.slug } }" class="block bg-gray-200">
            <img v-if="newsletter.coverImage" :src="newsletter.coverImage" :alt="newsletter.title" class="h-52 w-full object-contain" />
          </RouterLink>

          <div class="p-6">
            <p class="mb-3 text-sm text-gray-500">{{ formatDate(newsletter.pubDate) }}</p>

            <h2 class="mb-3 text-xl font-bold text-gray-900">
              <RouterLink :to="{ name: 'newsletter-detail', params: { slug: newsletter.slug } }" class="hover:text-democratic-red">
                {{ newsletter.title }}
              </RouterLink>
            </h2>
            ...
            <RouterLink :to="{ name: 'newsletter-detail', params: { slug: newsletter.slug } }" class="font-medium text-democratic-red hover:underline">
