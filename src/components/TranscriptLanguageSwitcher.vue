<template>
  <div class="relative">
    ...
    <div
      v-if="isOpen"
      class="absolute right-0 z-50 mt-2 w-48 rounded-lg border border-gray-300 bg-white shadow-lg"
    >
      <div class="py-1">
        <button
...
const currentLocaleFlag = computed(
  () => supportedLocales.find((l) => l.code === currentLocale.value)?.flag ?? "🌐",
);

const currentLocaleName = computed(
  () => supportedLocales.find((l) => l.code === currentLocale.value)?.name ?? "Unknown",
);
