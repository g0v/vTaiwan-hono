  ...
  <footer class="vt-footer-bg relative font-serif text-[#d8d8de]">
    ...
    <div class="mx-auto max-w-6xl px-6 pt-10 pb-6">
      <div class="grid grid-cols-1 gap-8 sm:grid-cols-[1.4fr_1fr_1fr]">
        ...
        <!-- 社群連結 -->
        <div>
          <div class="mb-2.5 flex h-7 items-center gap-2 font-sans text-[11px] font-semibold tracking-[0.14em] text-[#e4e4ea] uppercase">
      ...
      <div class="relative mt-7 flex flex-col items-center justify-between gap-x-5 gap-y-3 pt-4 font-sans text-xs text-[#76767e] md:flex-row">
        <span class="vt-hairline absolute inset-x-0 top-0 h-px" />
        <div class="flex flex-col items-center gap-x-3 gap-y-1.5 sm:flex-row">
          <a href="https://creativecommons.org/licenses/by-sa/4.0/deed.zh-hant" target="_blank" rel="noopener noreferrer" class="transition-colors hover:text-white">
            {{ t('footer.copyright', { year: currentYear }) }}
          </a>
          <span class="opacity-30">·</span>
          <div>
            <template v-for="(item, i) in legal" :key="item.labelKey">
              <RouterLink v-if="item.to" :to="item.to" class="text-[#b6b6be] transition-colors hover:text-white">
                {{ t(item.labelKey) }}
              </RouterLink>
              <a v-else :href="item.href" target="_blank" rel="noopener noreferrer" class="text-[#b6b6be] transition-colors hover:text-white">
                {{ t(item.labelKey) }}
              </a>
