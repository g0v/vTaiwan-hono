<template>
  <div class="transcript-panel flex h-full flex-col border-l border-gray-200 bg-white">
      ...
      <div class="mb-3">
        <div class="flex items-center gap-2">
          ...
          <input
            ...
            v-model="localDate"
            @change="onDateChange"
            class="rounded-sm border border-gray-300 px-2 py-1 text-sm focus:ring-2 focus:ring-jade-green focus:outline-hidden"
            :max="todayDate"
          />
        </div>
      </div>
    ...
    <div ref="transcriptContent" class="max-h-[50vh] shrink-0 space-y-2 overflow-y-auto p-4" :class="fontSizeClass">
      <div v-if="Object.keys(transcriptDataEntries).length === 0" class="py-8 text-center text-gray-500">
        <IconWrapper name="file-text" :size="48" class="mx-auto mb-4 text-gray-300" />
        <p>{{ $t('transcript.noContent') }}</p>
      </div>

      <div v-for="(entry, index) in Object.values(transcriptDataEntries)" :key="entry.timestamp" class="transcript-entry rounded-lg border border-gray-200 p-3 transition hover:border-gray-300">
