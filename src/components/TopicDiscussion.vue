...
const chineseSort = (a: string, b: string): number => {
  ...
  const chineseToNumber = (str: string) => str.replace(/一|二|三|四|五|六|七|八|九|十/gi, m => c2n[m] || m)
  ...
  try {
    ...
    if (link.indexOf('pol.is') > -1) {
      ...
      try {
        ...
        discussionType.value = {
          type: 'discourse',
          embeder: sortedTopics.filter(topic => topic.id && topic.title).map(topic => ({ title: topic.title, id: topic.id })),
        }
      } catch (error) {
  ...
  <div class="topic-discussion">
    ...
    <div v-else-if="discussionType.type">
      <div v-if="discussionType.type === 'discourse'" class="space-y-6">
        <div v-for="(disc, index) in discussionType.embeder as DiscourseEmbedItem[]" :key="index" class="rounded-lg border border-gray-200 bg-white shadow-xs">
