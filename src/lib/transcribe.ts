...
export async function translateToZh(
  ...
  const twResponse = await aiRunner(env.AI!).run("@cf/openai/gpt-oss-20b", {
    instructions:
      "請把以下內容修成台灣常用正體中文用語，避免中國大陸地區慣用語，只在必要時替換詞彙與語序，不要改變意思。",
    input: translatedText,
  });
  return extractGptOssText(twResponse);
}

export async function readAudioToText(
  ...
  try {
    ...
    const isHallucination =
      (trimmed.startsWith("字幕志願者") ||
        trimmed.startsWith("字幕志愿者") ||
        trimmed.includes("感謝觀看") ||
        trimmed.includes("謝謝觀看") ||
        trimmed.includes("打賞支持")) &&
      trimmed.length < 40;
