...
async function generateChunkSummary(chunk: string, env: AppBindings, chunkIndex: number, totalChunks: number): Promise<string> {
  const prompt = totalChunks > 1 ? `請為以下第${chunkIndex + 1}/${totalChunks}段內容生成重點摘要：` : `請用正體中文把以下內容整理出來，重點整理。：`
...
async function processChunks(chunks: string[], env: AppBindings): Promise<string> {
  ...
  const summaryPromises = chunks.map((chunk, i) => generateChunkSummary(chunk, env, i, chunks.length))
