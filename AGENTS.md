...
`src/api/*.ts` 每個模組各自 `new Hono<AppEnv>()`、路由寫**相對路徑**、`export default app`；前綴一律由 `src/index.ts` 的 `app.route()` 指定。加新端點就是「在模組裡加一條相對路徑的路由」，不必動 `index.ts`。
...
- **本站的 `corsFor` 一律不帶 `credentials`**（見 `cors.ts`），所以 `Access-Control-Allow-Credentials` 永遠不會出現，**任何跨來源讀取都是匿名的**。需要 session 的 GET 端點（`/api/admin/audit-log`、`/api/transcription/:id/versions`）跨站只會拿到 401。⚠️ 在 `corsFor` 加上 `credentials: true` 等於把 session 資料開放給 `ALLOWED_ORIGINS` 上的每個網域——`api-csrf.test.ts` 有測試會擋下這個改動。
- **`/api/admin/audit-log` 連 `corsFor` 都不掛**，跨來源根本讀不到；後台自家端點都該比照。

⚠️ **唯一需要另外想清楚的 GET：WebSocket 升級 `/api/meeting/ws/:date`。** WebSocket 握手不受同源政策約束、瀏覽器也不會做 preflight，所以跨站頁面**開得起這條連線**。它不是跨站寫入破口的原因是 **session cookie 為 `SameSite=Lax`**（Better Auth 預設，`createAuth.ts` 未覆寫；step-up cookie 也明寫 `sameSite: 'lax'`）——跨站發起的握手不是頂層導覽，cookie 不會送出，Worker 因此帶 `X-Authenticated: false`，DO 的 `webSocketMessage` 直接拒絕寫入。跨站連上來的效果等同匿名觀眾，而會議逐字稿本來就是公開可讀（`GET /api/meeting/:date` 不需認證）。**要動 cookie 的 `sameSite`（例如為了跨子網域登入改成 `none`）就必須同時在 WS 升級處加 Origin 檢查**，否則會開出跨站寫入。

模組內部也可以再 `app.route()` 巢狀掛載（例如 `admin.route('/civic-talks', civicTalk)`），讓「這組端點屬於哪個前綴」在程式碼結構上是顯性的。⚠️ 子 app 內的邏輯若要比對絕對路徑（`src/api/auth.ts` 的 `requiresStepUp(pathname)`、`update-user` 判斷），那份字串就綁死了 `index.ts` 的掛載點——改掛載前綴時要一併改，`step-up.test.ts` 有端點層級的斷言會抓到。
...
| API 端點／掛載結構（`src/api/*.ts`、`index.ts` 的 `app.route()`）                                                 | `vp check` + `vp test`（`api-routing.test.ts` 釘住中介層順序與動態路徑收窄、`api-csrf.test.ts` 釘住各掛載點的跨站防護）；見「API 模組掛載規則」 |
...
| 區塊             | 落地位置 / 說明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Auth 端點        | `/api/auth/*`（Better Auth handler，前置 `requiresStepUp` 檢查）+ `/api/auth/me`（回 `AuthContext`，註冊在 Better Auth catch-all 之前），見 `src/api/auth.ts`（薄層）；實際邏輯在 `src/server/lib/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| 跨站（CSRF）防護 | 全域 `app.use('/api/*', csrf())`（`hono/csrf`，見 `src/index.ts`）——**唯一**的同源把關點，逐端點的 `hasSameOrigin` 已移除；`/api/auth/*` 另有 Better Auth 自身的 `trustedOrigins` 檢查。⚠️ 無 Origin／無 Sec-Fetch-Site 的表單型請求一律 403（非瀏覽器請求不再豁免），受影響者為 `/api/transcription/create-table`（curl 要帶 `Content-Type: application/json`）與 `/api/transcription/test-ai`（multipart，僅剩同源瀏覽器打得到）；未來加 `form_post` 模式的 OAuth provider（如 Apple）其跨站 POST callback 也會被擋。`api-csrf.test.ts` 以真實 Hono app 驗證。⚠️ **`cors.ts` 的 `ALLOWED_ORIGINS` 只對 GET 端點生效**——跨來源的 `OPTIONS` preflight 不是安全方法、又沒有 `Content-Type`（`hono/csrf` 視同 `text/plain`），所以連 preflight 都會被 csrf 擋成 403，`corsFor(['POST'])` 的標頭永遠送不出去（實測白名單內的 `https://vtaiwan.tw` → `https://next.vtaiwan.tw` 亦然）。因此**寫入端點一律不掛 `corsFor`**（`api-routing.test.ts` 的「寫入端點不對外宣告 CORS」逐條釘住），掛了只會讓 `ALLOWED_ORIGINS` 誤導後人以為那些路徑可以跨站呼叫。真的需要跨站寫入必須先動全域 csrf 設定，那是要與使用者確認的決定                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
