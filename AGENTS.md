...
| Social 登入          | 🚧 code 進、待實測 | `createAuth.ts` 設 Google + GitHub provider；client 端 Google／GitHub 登入（`SocialLogin.vue`、`authClient.ts`、`App.vue`；GitHub 選項見 #75）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | #66 open |
...
| 變更日誌落地        | 🚧 code 進、待實測          | **事件來源＝D1 審計表 `admin_audit_log`**（放在 `DB`／`vtaiwan-transcriptions`，見 `migrations/0002_add_admin_audit_log.sql`；`DB_AUTH` 的 schema 由 CLI 生成不得手寫）。涵蓋**角色／停權變更＋逐字稿上傳／覆蓋＋大綱編輯**：前者寫在 `src/api/auth.ts`（成功才入帳，操作對象姓名與原角色在動作生效前快照——跨 D1 無法 JOIN），後者寫在 `src/api/transcription.ts`。schema 通用（`action` + `target` + JSON `detail`），加事件種類不必改表。讀取端 `GET /api/admin/audit-log` **僅 super-admin 且需 session 新鮮**（日誌含成員姓名／信箱，敏感度等同成員列表）。`recordAudit` **絕不 throw**：日誌寫失敗不得把已成功的管理操作變成 500。**所有會改變狀態的 admin 端點皆已入帳**（含 `impersonate-user`）；其中兩個的操作對象不在 request body 的 `userId`，由 `src/api/auth.ts` 另外取得（#74）——`create-user` 讀成功回應的 `{ user }`（該分支刻意讀完 body 再以相同內容重建 `Response`，不 tee 串流），`revoke-user-session` 則在動作生效前以 `sessionToken` 反查 `DB_AUTH` 的 `session` 表。⚠️ 事件名 `user.session.revoke`（單一工作階段）與 `user.sessions.revoke`（全部工作階段）只差一個字母、端點路徑互為前綴，`audit-log.test.ts` 有相鄰斷言釘住，勿調換。`detail.reason`（停權理由）有寫入但尚未在 UI 呈現——待 #70 的停權介面一起做。⚠️ **表要先建**：`DB` 綁定為 `remote`，`migrations/0002` 未套用時 `listAudit` 會 500（Worker log 有提示字串）、`recordAudit` 則靜默失敗；套 migration 或打 `/api/create-table` 皆可（idempotent）。**「管理操作」欄**：逐字稿／大綱的變更列附「回復到變更前版本」按鈕，`transcription.create` 列則是「刪除此逐字稿」（變更前＝不存在，使用者裁定）；哪一列有按鈕由純函式 `restoreCommandFor` 單一決定，前端顯示與端點呼叫共用。回復本身也入帳，因此可以再回復回去 | #71      |
...
| client 授權（`auth-session.ts`／`SocialLogin.vue`） | client 權限判斷**只是 UX**，不可當安全邊界——真正把關一律在 Worker 端；SSR 路徑不得讀 `document`／`localStorage`                                                                                                                                                                                                                                                             |
