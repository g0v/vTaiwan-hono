...
- **`/api/admin/audit-log` 連 `corsFor` 都不掛**，跨來源根本讀不到；後台自家端點都該比照。
...
| 跨站（CSRF）防護 | HTTP 寫入由全域 `app.use('/api/*', csrf())`（`hono/csrf`，見 `src/index.ts`）把關；`/api/auth/*` 另有 Better Auth 自身的 `trustedOrigins`。無 Origin／無 Sec-Fetch-Site 的表單型請求一律 403；未來若加 `form_post` OAuth callback 要另行處理。寫入端點一律不掛 `corsFor`：不回 preflight 所需標頭，瀏覽器就無法完成跨來源寫入；不要依賴特定 Hono 版本是否由 `csrf()` 擋 OPTIONS，也不要把 CORS 當授權。`api-routing.test.ts` 會先由 route table 確認案例完整涵蓋全部寫入路由，再逐條驗證。WebSocket `/api/meeting/ws/:date` 是 GET、沒有 CORS preflight，故在端點內另驗同源 `Origin`；`SameSite=Lax` 只區分 site，不能取代這道守衛。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
