...
2. **SSR 只出靜態殼，動態資料 hydration 後才抓。** 不在 Worker 端接外部 API 做動態 SSR；登入態一律由 client mount 後打 `/api/auth/me` 取得。要改這個策略，先與使用者確認。（Firebase 已於 #81 全數移除，專案不再有任何 Firebase 相依。）
...
- `src/server/api/` — 各 `/api/*` 端點（`auth`、`admin`＋`civic-talk`、`transcription`、`jitsi_token`、`meeting`、`proxy`、`mastodon`、`discourse_*`、`hello`）＋ `cors.ts`（白名單）、`types.ts`（`AppBindings`）。每個模組是獨立的 `new Hono<AppEnv>()`、`export default app`，路由寫**相對路徑**（`app.post('/upload', …)`），前綴由 `src/index.ts` 的 `app.route('/api/transcription', transcription)` 決定——這是 Hono 官方 [Building a larger application](https://hono.dev/docs/guides/best-practices#building-a-larger-application) 的寫法。詳見「API 模組掛載規則」。
...
`src/server/api/*.ts` 每個模組各自 `new Hono<AppEnv>()`、路由寫**相對路徑**、`export default app`；前綴一律由 `src/index.ts` 的 `app.route()` 指定。加新端點就是「在模組裡加一條相對路徑的路由」，不必動 `index.ts`。

有三條規則是**靜默失效型**的——違反時不會有錯誤訊息，只會失去防護或走到錯的 handler。`src/tests/api-routing.test.ts` 與 `api-csrf.test.ts` 釘住它們，動到掛載結構時務必跑 `vp test`。

1. **全域 `/api/*` 中介層必須註冊在 `app.route()` 區塊之前。** Hono 依註冊順序組 handler chain，子 app 的 handler 一回應就結束整條 chain——寫在 `app.route()` 之後的 `app.use('/api/*', …)` 對所有掛載的子 app 都不會執行。`csrf()` 是本站授權模型的第一道閘，掉出來等於全站失去跨站防護。
2. **所有 API 端點都要掛在 `/api/` 之下。** 掛在別的前綴就落在 `csrf()` 的守備範圍外。
3. **同層的動態路徑（`/:param`）不得與固定名稱端點形狀重疊。** 例如 `/api/transcription` 底下同時有 `/upload` 與 `/:lang`：`:lang` 已用 regex 收窄成 `LANG_MAP` key 的精確清單（`LANG_PATH`，由 `Object.keys(LANG_MAP)` 生成，兩者不會走鐘），所以順序無關；沒收窄的話，固定端點只要被加在動態路徑之後就會被靜默吃掉。新增動態路徑時一律加 regex 約束。

### GET 端點的跨站防線（為什麼 csrf 不管 GET）

`hono/csrf` 只擋非安全方法（GET／HEAD 一律放行），這是刻意的，不是缺口：

- **跨站 GET 擋不住也不該擋。** `<img>`、`<script>`、`<link>`、`<iframe>` 都能發跨站 GET，這是網頁平台的既有能力。真正的規則是**GET 必須是安全方法**——不改變狀態。GET 端點若會寫入，那才是要修的 bug，不是加 csrf 能補的。
- **跨站「讀」由同源政策把關，不是 csrf。** 攻擊者發得出請求，但讀不到回應——除非我們主動回 CORS 標頭。
- **`corsFor` 只掛在公開讀取的 GET 端點**（`hello`／`proxy`／`mastodon`／`discourse_*`／逐字稿列表與全文），寫入端點一律不掛——理由見上面 CSRF 欄。
- ⚠️ **`corsFor` 的掛載路徑要精確到單一端點，不要用 wildcard。** Hono 的尾綴 wildcard 連「零個區段」都算命中：`app.use('/:meeting_id/*', corsFor(…))` 會一併蓋到同層的 `/upload`、`/delete`、`/:lang` 等寫入端點。寫入端點的 handler 註冊在 middleware 之前、先回應就結束 chain，所以**實際請求看不出異狀**，只有沒有 handler 的 `OPTIONS` 會落到那個 corsFor 上並回出放行標頭。Hono >= 4.13.0 起 `csrf()` 把 `OPTIONS` 列入安全方法（`isSafeMethodRe` 加了 `OPTIONS`），不再替我們攔下 preflight——這正是「不要依賴特定 Hono 版本是否由 `csrf()` 擋 OPTIONS」的實例。`api-routing.test.ts` 對每條寫入路由各驗一次 preflight。
- **本站的 `corsFor` 一律不帶 `credentials`**（見 `cors.ts`），所以 `Access-Control-Allow-Credentials` 永遠不會出現，瀏覽器不會把 credentialed response 暴露給跨來源 script。這不代表 request 一定沒帶 cookie：是否送出由 Fetch credentials mode 與 cookie policy 決定，CORS 管的是 response 能否交給 script。⚠️ 在 `corsFor` 加上 `credentials: true` 等於把 session 資料開放給 `ALLOWED_ORIGINS` 上的每個網域——`api-csrf.test.ts` 有測試會擋下這個改動。
- **`/api/admin/audit-log` 連 `corsFor` 都不掛**，跨來源根本讀不到；後台自家端點都該比照（逐字稿的 `/:meeting_id/versions*` 為管理員專屬讀取，同樣不掛）。

⚠️ **WebSocket 升級 `/api/meeting/ws/:date` 必須獨立驗 Origin。** WebSocket 握手是 GET，不受 `csrf()` 保護，也沒有瀏覽器 CORS preflight；瀏覽器還會以 `credentials: include` 建立握手。`SameSite=Lax` 只區分 site，不區分 origin，因此同一 registrable domain 下的惡意子網域仍可能帶到 session cookie，不能把 SameSite 當成完整防線。Worker 會在讀 session 前拒絕非同源 `Origin`；缺少 Origin 的非瀏覽器 client 仍可匿名連線。這條守衛由 `api-csrf.test.ts` 釘住，調整 cookie 或 WS 來源政策時必須同步更新。

模組內部也可以再 `app.route()` 巢狀掛載（例如 `admin.route('/civic-talks', civicTalk)`），讓「這組端點屬於哪個前綴」在程式碼結構上是顯性的。⚠️ 子 app 內的邏輯若要比對絕對路徑（`src/server/api/auth.ts` 的 `requiresStepUp(pathname)`、`update-user` 判斷），那份字串就綁死了 `index.ts` 的掛載點——改掛載前綴時要一併改，`step-up.test.ts` 有端點層級的斷言會抓到。
...
| API 端點／掛載結構（`src/server/api/*.ts`、`index.ts` 的 `app.route()`）                                                        | `vp check` + `vp test`（`api-routing.test.ts` 釘住中介層順序與動態路徑收窄、`api-csrf.test.ts` 釘住各掛載點的跨站防護）；見「API 模組掛載規則」 |
| 登入／授權／Admin（`src/server/lib/`、`src/server/api/auth.ts`、`src/server/api/admin.ts`、`AdminView.vue`、`auth-session.ts`） | 見「Admin 介面與 Better Auth」的 **B. 專屬驗收檢查**；同時更新 `design/登入與權限管理工程示意圖.svg`                                            |
...
| 區塊             | 落地位置 / 說明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Auth 端點        | `/api/auth/*`（Better Auth handler，前置 `requiresStepUp` 檢查）+ `/api/auth/me`（回 `AuthContext`，註冊在 Better Auth catch-all 之前），見 `src/server/api/auth.ts`（薄層）；實際邏輯在 `src/server/lib/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 跨站（CSRF）防護 | HTTP 寫入由全域 `app.use('/api/*', csrf())`（`hono/csrf`，見 `src/index.ts`）把關；`/api/auth/*` 另有 Better Auth 自身的 `trustedOrigins`。無 Origin／無 Sec-Fetch-Site 的表單型請求一律 403；未來若加 `form_post` OAuth callback 要另行處理。寫入端點一律不掛 `corsFor`：不回 preflight 所需標頭，瀏覽器就無法完成跨來源寫入；不要依賴特定 Hono 版本是否由 `csrf()` 擋 OPTIONS（Hono >= 4.13.0 已把 OPTIONS 視為安全方法而放行 preflight），也不要把 CORS 當授權。`api-routing.test.ts` 會先由 route table 確認案例完整涵蓋全部寫入路由，再逐條驗證實際請求與 preflight 都不帶 CORS 放行標頭。WebSocket `/api/meeting/ws/:date` 是 GET、沒有 CORS preflight，故在端點內另驗同源 `Origin`；`SameSite=Lax` 只區分 site，不能取代這道守衛。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 端點權限強制     | `jitsi-token`（`meeting.join`；moderator claim 由 `meeting.moderate` 決定）、`/api/meeting/*`（含 WebSocket `/api/meeting/ws/:date`，`meeting.join`）、逐字稿寫入與版本讀取（`transcription.update`）、`/api/transcription/:lang`（`meeting.join`；**站內專用**，不掛 `corsFor`，語言碼由 `LANG_MAP` 的 key 生成精確清單）。Better Auth admin plugin 端點 `/api/auth/admin/*`（list-users／set-role／ban-user…）由 plugin 自身強制（未登入 401、權限不足 403），設定單一來源為 `createAuth.ts` 的 `adminRoleAccess`，Worker 端不再疊平行守衛。**每個 `Permission` 都有實際強制它的端點，不留孤兒權限**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 變更日誌           | **事件來源＝D1 審計表 `admin_audit_log`**（放在 `DB`／`vtaiwan-transcriptions`，見 `migrations/0002_add_admin_audit_log.sql`；`DB_AUTH` 的 schema 由 CLI 生成不得手寫）。涵蓋**角色／停權變更＋逐字稿上傳／覆蓋／回復／刪除＋大綱編輯**：前者由 `createAuth.ts` 的 `before`／`after` hook 觸發、實作在 `src/server/lib/auth-audit.ts`（`before` 於動作生效前快照操作對象姓名與原角色——跨 D1 無法 JOIN；`after` 確認回應成功才入帳），後者寫在 `src/server/api/transcription.ts`。schema 通用（`action` + `target` + JSON `detail`），加事件種類不必改表。讀取端 `GET /api/admin/audit-log` **僅 super-admin 且需 session 新鮮**（日誌含成員姓名／信箱，敏感度等同成員列表）。`recordAudit` **絕不 throw**：日誌寫失敗不得把已成功的管理操作變成 500。**所有會改變狀態的 admin 端點皆已入帳**（含 `impersonate-user`）；其中兩個的操作對象不在 request body 的 `userId`，由 `auth-audit.ts` 另外取得（#74）——`create-user` 讀成功回應的 `{ user }`，`revoke-user-session` 則在動作生效前以 `sessionToken` 反查 `DB_AUTH` 的 `session` 表（`findAuditUserIdBySessionToken`）。⚠️ 事件名 `user.session.revoke`（單一工作階段）與 `user.sessions.revoke`（全部工作階段）只差一個字母、端點路徑互為前綴，`audit-log.test.ts` 有相鄰斷言釘住，勿調換。`detail.reason` 記錄停權理由。⚠️ **表要先建**：`DB` 綁定為 `remote`，`migrations/0002` 未套用時 `listAudit` 會 500（Worker log 有提示字串）、`recordAudit` 則靜默失敗；套 migration 或打 `/api/transcription/create-table` 皆可（idempotent）。**「管理操作」欄**：逐字稿／大綱的變更列附「回復到變更前版本」按鈕，`transcription.create` 列則是「刪除此逐字稿」（變更前＝不存在，使用者裁定）；哪一列有按鈕由純函式 `restoreCommandFor` 單一決定，前端顯示與端點呼叫共用。回復本身也入帳，因此可以再回復回去 |
...
| 新增受保護端點                                      | 三道關卡齊全：全域 `csrf()`（掛在 `/api/*`，**不要**在端點內自己再寫同源檢查）→ `getAuthContext`（401）→ `hasPermission`（403）；缺一即為授權破口。第一道閘還有個前提——`csrf()` 必須註冊在 `index.ts` 的 `app.route()` 區塊之前，且新端點掛在 `/api/` 之下（見「API 模組掛載規則」）。若屬敏感操作，最後再加 `context.fresh`（403 + `SESSION_NOT_FRESH`），並在 `step-up.test.ts` 補上對應斷言       |
...
| 新增後台變更事件（`AuditAction`）                   | 在 `src/lib/audit-log.ts` 加值後**三件事一起做**：`ACTION_LABEL_KEYS` 補對應、l10n 三檔補 `admin.logs.action.*`、`audit-log.test.ts` 的 `ALL_ACTIONS` 補上（該測試會抓到缺文案）。動到 `admin_audit_log` 欄位時，`migrations/0002_*.sql` 與 `/api/transcription/create-table` 的 bootstrap SQL **兩邊都要改**。新增會改變狀態的管理／內容端點時一併呼叫 `recordAudit`，否則該變更不會留痕            |
